...
const DataBase: React.FunctionComponent<DataBaseProps> = ({
  ...
  const [matchedSelected, setMatchedSelected] = useState<number[]>([]);
  const [dataBases, setDataBases] = useState<{ data: string[];dataBaseTable: any[] }>({ data: [], dataBaseTable });
  const [selectedCell, setSelectedCell] = useState<number | null>(null);
  const [editOpen, setEditOpen] = useState(false);
  const [deleteOpen, setDeleteOpen] = useState(false);
  const theme = useTheme();
  ...
  const addCustomField = (custom: string) => {
    setDataBases({ ...dataBases, data: [...dataBases.data, custom] });
  };
  ...
  const handleNextTab = () => {
    const transformedTableData = dataBases.dataBaseTable.reduce((acc, item) => {
      acc[item.name] = companyFormData[item.name] || "no";
      return acc;
    }, {});
    setCompanyFormData((prevData: any) => ({
            ...
            <Box sx={{
              ...
              <Box sx={{ mt: "10px", overflowX: "auto" }}>
                <Table borderAxis="both" >
                  ...
                  <tbody>
                    {dataBases.dataBaseTable.map((data, index) => (
                        <tr key={index}>
                          <td>
                            <Checkbox />
                          </td>
