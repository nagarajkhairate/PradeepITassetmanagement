...
export const MaintenancesDue: React.FC = () => {
  ...
  useEffect(() => {
    if (location.state && location.state.selectedColumns) {
      const columnNames = location.state.selectedColumns;
      const columns = maintenanceDatabase.filter(col => columnNames.includes(col.fieldName));
      setSelectedColumns(columns);
    }
  }, [location.state, maintenanceDatabase]);

  useEffect(() => {
    if (maintenanceDatabase.length > 0) {
      const columnsFromPreviousSelection = maintenanceDatabase.filter(
        (column) => column.isTable,
      )
      setSelectedColumns(columnsFromPreviousSelection)
    }
  }, [maintenanceDatabase])
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* <tbody>
              {alertsMaintenanceDue &&
                alertsMaintenanceDue
                  .filter((field: any) => field.isTable)
                  .map((lease: any, rowIndex: number) => (
                    <tr key={rowIndex}>
                      {selectedColumns.map((col) => (
                        <td
                          key={col.id}
